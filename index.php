...
<body class="grid md:grid-cols-2 gap-0 place-items-center p-6 md:p-10">
  ...
  <section class="hidden md:flex w-full h-full items-center justify-center">
    <div class="max-w-lg text-white px-6 reveal">
      ...
      <h1 class="text-4xl font-extrabold leading-tight tracking-tight">
        ATIERA <span class="text-gold">Financial</span> Management
      </h1>
  ...
  <main class="w-full max-w-md md:ml-auto">
    <div id="card" class="card p-6 sm:p-8 reveal">
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400 mb-5">Use your credentials to continue.</p>

      <!-- Alerts -->
      <?php if (isset($_SESSION['error'])): ?>
        <div class="alert alert-error mb-3"><?php echo $_SESSION['error']; unset($_SESSION['error']); ?></div>
      <?php endif; ?>

      <form id="loginForm" action="login.php" method="POST" class="space-y-4">
        <!-- Username -->
        <div class="field">
          <input id="username" name="username" type="text" autocomplete="username" class="input peer" placeholder=" " required>
        ...
        <div>
          ...
          <div class="field">
            ...
            <div class="icon-right flex items-center gap-1">
              <button type="button" id="togglePw" class="w-9 h-9 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center" aria-label="Show password" aria-pressed="false">
...
<script>
  ...
  if (modeBtn){
    modeBtn.addEventListener('click', () => {
      ...
      if (wmImg){ wmImg.style.transform = 'scale(1.01)'; setTimeout(()=> wmImg.style.transform = '', 220); }
    });
  }
